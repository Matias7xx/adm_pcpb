...
watch(() => form.valores_dinheiro, (novoValor) => {
...
watch(() => form.operacao_id, (novoId) => {
  if (novoId && props.operacoesDisponiveis) {
    const opSelecionada = props.operacoesDisponiveis.find(op => op.id === novoId);
    if (opSelecionada) {
      form.autoridade_responsavel_nome = opSelecionada.autoridade_responsavel_nome || '';
      form.autoridade_responsavel_matricula = opSelecionada.autoridade_responsavel_matricula || '';
...
watch(() => form.tipo_arma_apreendida, (novoValor) => {
...
watch(() => form.entorpecente_apreendido, (novoValor) => {
...
const campoNumericoInvalido = (valor) => {
  return valor === null || valor === undefined || valor === '' || isNaN(valor) || valor < 0;
};
...
const validarEtapa = (etapa) => {
  ...
  if (etapa === 1) {
    ...
    if (!form.autoridade_responsavel_nome?.trim()) erros.autoridade_responsavel_nome = 'Nome da autoridade é obrigatório.';
    if (!form.autoridade_responsavel_matricula?.trim()) erros.autoridade_responsavel_matricula = 'Matrícula da autoridade é obrigatória.';
  }

  if (etapa === 2) {
    // Validação robusta: verifica se está vazio E se é negativo
    if (campoNumericoInvalido(form.mandados_prisao_cumpridos)) {
      erros.mandados_prisao_cumpridos = 'Este campo é obrigatório (use 0 se não houver).';
    }
    ...
    if (campoNumericoInvalido(form.mandados_prisao_nao_cumpridos)) {
      erros.mandados_prisao_nao_cumpridos = 'Este campo é obrigatório (use 0 se não houver).';
    }
    if (campoNumericoInvalido(form.mandados_busca_cumpridos)) {
      erros.mandados_busca_cumpridos = 'Este campo é obrigatório (use 0 se não houver).';
    }
    if (campoNumericoInvalido(form.mandados_busca_infrator_cumpridos)) {
      erros.mandados_busca_infrator_cumpridos = 'Este campo é obrigatório (use 0 se não houver).';
    }
    if (campoNumericoInvalido(form.mandados_busca_infrator_nao_cumpridos)) {
      erros.mandados_busca_infrator_nao_cumpridos = 'Este campo é obrigatório (use 0 se não houver).';
    }
    if (campoNumericoInvalido(form.prisoes_flagrante)) {
      erros.prisoes_flagrante = 'Este campo é obrigatório (use 0 se não houver).';
    }
  }
  ...
  if (etapa === 4) {
    ...
    if (!form.entorpecente_apreendido.includes('NENHUM') && form.entorpecente_apreendido.length > 0 && !form.detalhes_entorpecentes?.trim()) {
  ...
  if (etapa === 5) {
    ...
    if (campoNumericoInvalido(form.valores_dinheiro)) {
      erros.valores_dinheiro = 'Este campo é obrigatório (use 0 se não houver).';
    }
        ...
        <form @submit.prevent="submit" novalidate>
          <div class="bg-white shadow rounded-lg p-6">
            ...
            <div v-show="etapaAtual === 1" class="space-y-6">
              ...
              <div v-if="!operacao">
                ...
                <select
                  ...
                  :class="erro('operacao_id') ? 'border-red-500' : 'border-gray-300'"
                >
              ...
              <div v-if="operacaoSelecionada || operacao" class="bg-gray-50 border-2 border-gray-200 rounded-lg p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div class="bg-white p-3 rounded border border-gray-200">
                    ...
                    <p class="text-gray-900 font-medium mt-1">{{ operacao?.nome_operacao || 'Carregando...' }}</p>
                  </div>

                  <div class="bg-white p-3 rounded border border-gray-200">
                    <label class="text-xs font-semibold text-gray-700 uppercase"> Data da Deflagração</label>
                    <p class="text-gray-900 font-medium mt-1">
                      {{ operacao?.data_operacao_formatada || formatarData(operacao?.data_operacao) || 'Carregando...' }}
                    </p>
                  </div>

                  <div class="bg-white p-3 rounded border border-gray-200">
                    <label class="text-xs font-semibold text-gray-700 uppercase"> Unidade Solicitante/Executora Vinculada à</label>
                    <p class="text-gray-900 font-medium mt-1">{{ operacao?.vinculada_unidade || 'Carregando...' }}</p>
                  </div>

                  <div class="bg-white p-3 rounded border border-gray-200">
                    <label class="text-xs font-semibold text-gray-700 uppercase"> Unidade Solicitante/Executora</label>
                    <p class="text-gray-900 font-medium mt-1">{{ operacao?.vinculada_unidade_especializada || 'Carregando...' }}</p>
                  </div>
                </div>
              </div>
              ...
              <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">
                  Número do Processo PJE <span class="text-gray-500 text-xs">(Opcional - "PREJUDICADO" se nacional)</span>
                </label>
                <input
                  ...
                  class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-[#bea55a] focus:border-transparent"
                  :class="erro('numero_processo_pje') ? 'border-red-500' : 'border-gray-300'"
                />
                <p v-if="erro('numero_processo_pje')" class="mt-1 text-sm text-red-600">
              ...
              <div class="bg-gray-50 border-2 border-gray-300 rounded-lg p-4">
                ...
                <p class="text-sm text-gray-700 mb-4">
                  Por padrão, usa a mesma autoridade informada no cadastro da operação. Edite se for diferente.
                </p>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    />
                    <p v-if="erro('autoridade_responsavel_nome')" class="mt-1 text-sm text-red-600">
                  ...
                  <div>
                    ...
                    />
                    <p v-if="erro('autoridade_responsavel_matricula')" class="mt-1 text-sm text-red-600">
            ...
            <div v-show="etapaAtual === 2" class="space-y-6">
              ...
              <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">
                  Quantidade de Mandados de Prisão Cumpridos <span class="text-red-500">*</span>
                </label>
                <input
                  ...
                  @keypress="apenasNumeros"
                  class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-[#bea55a] focus:border-transparent"
                  :class="erro('mandados_prisao_cumpridos') ? 'border-red-500' : 'border-gray-300'"
                />
                <p v-if="erro('mandados_prisao_cumpridos')" class="mt-1 text-sm text-red-600">
              ...
              <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">
                   Detalhes dos Presos (Nome e CPF) <span class="text-red-500">*</span>
                </label>
                <textarea
                  ...
                  class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-[#bea55a] focus:border-transparent"
                  :class="erro('mandados_prisao_cumpridos_detalhes') ? 'border-red-500' : 'border-gray-300'"
                ></textarea>
                <p class="mt-1 text-xs text-gray-500">Liste um preso por linha. Se não houver, informe "N/A".</p>
                <p v-if="erro('mandados_prisao_cumpridos_detalhes')" class="mt-1 text-sm text-red-600">
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">
                     Mandados de Prisão Não Cumpridos <span class="text-red-500">*</span>
                  </label>
                  <input
                    ...
                    @keypress="apenasNumeros"
                    class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-[#bea55a] focus:border-transparent"
                    :class="erro('mandados_prisao_nao_cumpridos') ? 'border-red-500' : 'border-gray-300'"
                    required
                  />
                  <p v-if="erro('mandados_prisao_nao_cumpridos')" class="mt-1 text-sm text-red-600">
                ...
                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">
                     Mandados de Busca Cumpridos <span class="text-red-500">*</span>
                  </label>
                  <input
                    ...
                    @keypress="apenasNumeros"
                    class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-[#bea55a] focus:border-transparent"
                    :class="erro('mandados_busca_cumpridos') ? 'border-red-500' : 'border-gray-300'"
                  />
                  <p v-if="erro('mandados_busca_cumpridos')" class="mt-1 text-sm text-red-600">
                ...
                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">
                     Mandados de Busca de Infrator Cumpridos <span class="text-red-500">*</span>
                  </label>
                  <input
                    ...
                    @keypress="apenasNumeros"
                    class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-[#bea55a] focus:border-transparent"
                    :class="erro('mandados_busca_infrator_cumpridos') ? 'border-red-500' : 'border-gray-300'"
                  />
                  <p v-if="erro('mandados_busca_infrator_cumpridos')" class="mt-1 text-sm text-red-600">
                ...
                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">
                     Mandados de Busca de Infrator Não Cumpridos <span class="text-red-500">*</span>
                  </label>
                  <input
                    ...
                    @keypress="apenasNumeros"
                    class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-[#bea55a] focus:border-transparent"
                    :class="erro('mandados_busca_infrator_nao_cumpridos') ? 'border-red-500' : 'border-gray-300'"
                  />
                  <p v-if="erro('mandados_busca_infrator_nao_cumpridos')" class="mt-1 text-sm text-red-600">
              ...
              <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">
                   Quantidade de Prisões em Flagrante <span class="text-red-500">*</span>
                </label>
                <input
                  ...
                  @keypress="apenasNumeros"
                  class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-[#bea55a] focus:border-transparent"
                  :class="erro('prisoes_flagrante') ? 'border-red-500' : 'border-gray-300'"
                />
                <p v-if="erro('prisoes_flagrante')" class="mt-1 text-sm text-red-600">
            ...
            <div v-show="etapaAtual === 3" class="space-y-6">
              ...
              <div class="bg-gray-50 border-2 border-gray-200 rounded-lg p-6">
                <label class="block text-lg font-semibold text-gray-900 mb-4">
                  Alguma arma foi apreendida nesta operação? <span class="text-red-500">*</span>
                </label>
                ...
                <div class="flex gap-6">
                  <label class="flex items-center cursor-pointer">
                    ...
                    />
                    <span class="ml-3 text-base font-medium text-gray-900">Não</span>
                  </label>
                  ...
                  <label class="flex items-center cursor-pointer">
                    <input
                      ...
                      class="w-5 h-5 text-[#bea55a] border-gray-300 focus:ring-[#bea55a]"
                    />
                    <span class="ml-3 text-base font-medium text-gray-900">Sim</span>
                  </label>
                </div>
              </div>

              <!-- CAMPOS (só se SIM) -->
              <div v-if="houveApreensaoArmas" class="space-y-6 border-2 border-gray-200 bg-gray-50 rounded-lg p-6">
                
                <!-- Quantidade -->
                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">
                    Quantidade Total de Armas <span class="text-red-500">*</span>
                  </label>
                  <input
                    ...
                    @keypress="apenasNumeros"
                    class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-[#bea55a] focus:border-transparent"
                    :class="erro('quantidade_armas_apreendidas') ? 'border-red-500' : 'border-gray-300'"
                  />
                  <p v-if="erro('quantidade_armas_apreendidas')" class="mt-1 text-sm text-red-600">
                ...
                <div>
                  ...
                  <div class="grid grid-cols-1 md:grid-cols-2 gap-3 bg-white p-4 rounded-lg border">
                    <div class="flex items-center">
                      <input type="checkbox" id="arma_revolver" value="REVÓLVER"
                        v-model="form.tipo_arma_apreendida"
                        class="w-4 h-4 text-[#bea55a] border-gray-300 rounded focus:ring-[#bea55a]"
                      />
                      <label for="arma_revolver" class="ml-2 text-sm text-gray-700 cursor-pointer">Revólver</label>
                    </div>

                    <div class="flex items-center">
                      <input type="checkbox" id="arma_pistola" value="PISTOLA"
                        v-model="form.tipo_arma_apreendida"
                        class="w-4 h-4 text-[#bea55a] border-gray-300 rounded focus:ring-[#bea55a]"
                      />
                      <label for="arma_pistola" class="ml-2 text-sm text-gray-700 cursor-pointer">Pistola</label>
                    </div>

                    <div class="flex items-center">
                      <input type="checkbox" id="arma_espingarda" value="ESPINGARDA"
                        v-model="form.tipo_arma_apreendida"
                        class="w-4 h-4 text-[#bea55a] border-gray-300 rounded focus:ring-[#bea55a]"
                      />
                      <label for="arma_espingarda" class="ml-2 text-sm text-gray-700 cursor-pointer">Espingarda</label>
                    </div>

                    <div class="flex items-center">
                      <input type="checkbox" id="arma_fuzil" value="FUZIL"
                        v-model="form.tipo_arma_apreendida"
                        class="w-4 h-4 text-[#bea55a] border-gray-300 rounded focus:ring-[#bea55a]"
                      />
                      <label for="arma_fuzil" class="ml-2 text-sm text-gray-700 cursor-pointer">Fuzil</label>
                    </div>

                    <div class="flex items-center">
                      <input type="checkbox" id="arma_artesanal" value="ARMA ARTESANAL"
                        v-model="form.tipo_arma_apreendida"
                        class="w-4 h-4 text-[#bea55a] border-gray-300 rounded focus:ring-[#bea55a]"
                      />
                      <label for="arma_artesanal" class="ml-2 text-sm text-gray-700 cursor-pointer">Arma Artesanal</label>
                    </div>

                    <div class="flex items-center">
                      <input type="checkbox" id="arma_explosivo" value="EXPLOSIVO"
                        v-model="form.tipo_arma_apreendida"
                        class="w-4 h-4 text-[#bea55a] border-gray-300 rounded focus:ring-[#bea55a]"
                      />
                      <label for="arma_explosivo" class="ml-2 text-sm text-gray-700 cursor-pointer">Explosivo</label>
                    </div>
                  ...
                  <p class="mt-2 text-xs text-gray-500">
                    Selecionados: {{ form.tipo_arma_apreendida.join(', ') || 'Nenhum' }}
                  </p>
                  <p v-if="erro('tipo_arma_apreendida')" class="mt-1 text-sm text-red-600">
                ...
                <div>
                  ...
                  <textarea
                    ...
                    class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-[#bea55a] focus:border-transparent"
                    :class="erro('detalhes_armas_apreendidas') ? 'border-red-500' : 'border-gray-300'"
                  ></textarea>
                  <p v-if="erro('detalhes_armas_apreendidas')" class="mt-1 text-sm text-red-600">
                ...
                <div>
                  ...
                  <textarea
                    ...
                    class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-[#bea55a] focus:border-transparent"
                    :class="erro('municoes_apreendidas') ? 'border-red-500' : 'border-gray-300'"
                  ></textarea>
                  <p v-if="erro('municoes_apreendidas')" class="mt-1 text-sm text-red-600">
              ...
              <!-- MENSAGEM quando NÃO -->
              <div v-else-if="houveApreensaoArmas === false" class="bg-gray-50 border border-gray-300 rounded-lg p-6 text-center">
                <svg class="w-16 h-16 mx-auto text-gray-400 mb-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                </svg>
                <p class="text-lg font-medium text-gray-700">Nenhuma arma foi apreendida</p>
              </div>
            </div>
            ...
            <div v-show="etapaAtual === 4" class="space-y-6">
              ...
              <div>
                <label class="block text-sm font-medium text-gray-700 mb-3">
                   Entorpecentes Apreendidos (Marque todos que se aplicam) <span class="text-red-500">*</span>
                </label>
                ...
                <div class="space-y-2 bg-gray-50 p-4 rounded-lg border border-gray-200">
                  <div class="flex items-center">
                    ...
                    />
                    <label for="entorp_nao" class="ml-2 text-sm text-gray-700 cursor-pointer font-medium" :class="algumEntorpecenteMarcado ? 'opacity-50' : ''">
                  ...
                  <div class="flex items-center">
                    <input
                      ...
                      class="w-4 h-4 text-[#bea55a] border-gray-300 rounded focus:ring-[#bea55a] disabled:opacity-50 disabled:cursor-not-allowed"
                    />
                    <label for="entorp_maconha" class="ml-2 text-sm text-gray-700 cursor-pointer" :class="nenhumEntorpecenteMarcado ? 'opacity-50' : ''">
                      Maconha
                    </label>
                  </div>
                  ...
                  <div class="flex items-center">
                    <input
                      ...
                      class="w-4 h-4 text-[#bea55a] border-gray-300 rounded focus:ring-[#bea55a] disabled:opacity-50 disabled:cursor-not-allowed"
                    />
                    <label for="entorp_cocaina" class="ml-2 text-sm text-gray-700 cursor-pointer" :class="nenhumEntorpecenteMarcado ? 'opacity-50' : ''">
                      Cocaína
                    </label>
                  </div>
                  ...
                  <div class="flex items-center">
                    <input
                      ...
                      class="w-4 h-4 text-[#bea55a] border-gray-300 rounded focus:ring-[#bea55a] disabled:opacity-50 disabled:cursor-not-allowed"
                    />
                    <label for="entorp_crack" class="ml-2 text-sm text-gray-700 cursor-pointer" :class="nenhumEntorpecenteMarcado ? 'opacity-50' : ''">
                      Crack
                    </label>
                  </div>
                  ...
                  <div class="flex items-center">
                    <input
                      ...
                      class="w-4 h-4 text-[#bea55a] border-gray-300 rounded focus:ring-[#bea55a] disabled:opacity-50 disabled:cursor-not-allowed"
                    />
                    <label for="entorp_skank" class="ml-2 text-sm text-gray-700 cursor-pointer" :class="nenhumEntorpecenteMarcado ? 'opacity-50' : ''">
                      Skank
                    </label>
                  </div>
                  ...
                  <div class="flex items-center">
                    <input
                      ...
                      class="w-4 h-4 text-[#bea55a] border-gray-300 rounded focus:ring-[#bea55a] disabled:opacity-50 disabled:cursor-not-allowed"
                    />
                    <label for="entorp_heroina" class="ml-2 text-sm text-gray-700 cursor-pointer" :class="nenhumEntorpecenteMarcado ? 'opacity-50' : ''">
                      Heroína
                    </label>
                  </div>
                  ...
                  <div class="flex items-center">
                    <input
                      ...
                      class="w-4 h-4 text-[#bea55a] border-gray-300 rounded focus:ring-[#bea55a] disabled:opacity-50 disabled:cursor-not-allowed"
                    />
                    <label for="entorp_lsd" class="ml-2 text-sm text-gray-700 cursor-pointer" :class="nenhumEntorpecenteMarcado ? 'opacity-50' : ''">
                      LSD
                    </label>
                  </div>
                  ...
                  <div class="flex items-center">
                    <input
                      ...
                      class="w-4 h-4 text-[#bea55a] border-gray-300 rounded focus:ring-[#bea55a] disabled:opacity-50 disabled:cursor-not-allowed"
                    />
                    <label for="entorp_ecstasy" class="ml-2 text-sm text-gray-700 cursor-pointer" :class="nenhumEntorpecenteMarcado ? 'opacity-50' : ''">
                      Ecstasy (MDMA)
                    </label>
                  </div>
                  ...
                  <div class="flex items-center">
                    <input
                      ...
                      class="w-4 h-4 text-[#bea55a] border-gray-300 rounded focus:ring-[#bea55a] disabled:opacity-50 disabled:cursor-not-allowed"
                    />
                    <label for="entorp_outros" class="ml-2 text-sm text-gray-700 cursor-pointer" :class="nenhumEntorpecenteMarcado ? 'opacity-50' : ''">
                      Outros
                    </label>
                  </div>
                </div>
                ...
                <p class="mt-2 text-xs text-gray-500">
                  Selecionados: {{ form.entorpecente_apreendido.length > 0 ? form.entorpecente_apreendido.join(', ') : 'Nenhum' }}
                </p>
                
                <p v-if="erro('entorpecente_apreendido')" class="mt-1 text-sm text-red-600">
                  {{ erro('entorpecente_apreendido') }}
                </p>
              </div>

              <!-- 24. Detalhes dos Entorpecentes -->
              <div v-if="!form.entorpecente_apreendido.includes('NENHUM') && form.entorpecente_apreendido.length > 0">
                <label class="block text-sm font-medium text-gray-700 mb-2">
                   Especificar Peso/Quantidade dos Entorpecentes <span class="text-red-500">*</span>
                </label>
                <textarea
                  ...
                  class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-[#bea55a] focus:border-transparent"
                  :class="erro('detalhes_entorpecentes') ? 'border-red-500' : 'border-gray-300'"
                ></textarea>
                <p class="mt-1 text-xs text-gray-500">Especifique peso em gramas ou quantidade de unidades para cada tipo.</p>
                <p v-if="erro('detalhes_entorpecentes')" class="mt-1 text-sm text-red-600">
            ...
            <div v-show="etapaAtual === 5" class="space-y-6">
              ...
              <div>
                ...
                <input
                  ...
                  @keypress="apenasNumeros"
                  class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-[#bea55a] focus:border-transparent"
                  :class="erro('valores_dinheiro') ? 'border-red-500' : 'border-gray-300'"
                />
                
                <!-- Exibição formatada -->
                <p class="mt-2 text-sm font-medium text-gray-700">
                  Valor formatado: <span class="text-[#bea55a]">{{ form.valores_dinheiro_formatado }}</span>
                </p>
                
                <p class="mt-1 text-xs text-gray-500">Informe 0 se não houver valores apreendidos.</p>
                <p v-if="erro('valores_dinheiro')" class="mt-1 text-sm text-red-600">
              ...
              <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">
                   Veículos Apreendidos (Marca/Modelo/Placa/Ano/Cor/Valor FIPE) <span class="text-red-500">*</span>
                </label>
                <textarea
                  ...
                  class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-[#bea55a] focus:border-transparent"
                  :class="erro('veiculos_apreendidos') ? 'border-red-500' : 'border-gray-300'"
                ></textarea>
                <p class="mt-1 text-xs text-gray-500">Liste um veículo por linha com todos os dados. Se não houver, informe "N/A".</p>
                <p v-if="erro('veiculos_apreendidos')" class="mt-1 text-sm text-red-600">
              ...
              <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">
                   Demais Objetos Apreendidos (Atribuir Valor Estimado) <span class="text-red-500">*</span>
                </label>
                <textarea
                  ...
                  class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-[#bea55a] focus:border-transparent"
                  :class="erro('demais_objetos_apreendidos') ? 'border-red-500' : 'border-gray-300'"
                ></textarea>
                <p class="mt-1 text-xs text-gray-500">Liste objetos com valores estimados. Se não houver, informe "N/A".</p>
                <p v-if="erro('demais_objetos_apreendidos')" class="mt-1 text-sm text-red-600">
              ...
              <div>
                ...
                <textarea
                  ...
                  class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-[#bea55a] focus:border-transparent"
                  :class="erro('outras_informacoes') ? 'border-red-500' : 'border-gray-300'"
                ></textarea>
                <p v-if="erro('outras_informacoes')" class="mt-1 text-sm text-red-600">
