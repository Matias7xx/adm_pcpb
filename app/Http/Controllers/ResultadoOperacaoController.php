...
class ResultadoOperacaoController extends Controller
{
    ...
    private function temPermissao($resultado): bool
    {
        ...
        if ($resultado instanceof ResultadoOperacao) {
            return mb_strtolower(trim($resultado->unidade_policial_responsavel ?? ''))
    ...
    public function create(Request $request)
    {
        ...
        if ($request->filled('operacao_id')) {
            ...
            if (!$this->temPermissao($operacao)) {
                abort(403, 'Você não tem permissão para registrar resultado desta operação.');
            }

            // Verificar se já existe resultado para esta operação
            if ($operacao->resultado()->exists()) {
                return redirect()->route('resultados-operacao.show', $operacao->resultado->id)
                    ->with('info', 'Esta operação já possui resultado cadastrado.');
            }
        }

        // Buscar todas as operações da unidade que ainda não têm resultado
        $operacoesDisponiveis = Operacao::where('unidade_policial_responsavel', auth()->user()->lotacao)
    ...
    public function store(ResultadoOperacaoRequest $request)
    {
        ...
        return redirect()->route('resultados-operacao.show', ['resultado' => $resultado->id])
     ...
     */
    public function update(ResultadoOperacaoRequest $request, ResultadoOperacao $resultado)
    {
        if (!$this->temPermissao($resultado)) {
            abort(403, 'Você não tem permissão para editar este resultado.');
        }
    ...
    public function destroy(ResultadoOperacao $resultado)
    {
        ...
        return redirect()->route('resultados-operacao.index')
    ...
    public function gerarPdf(ResultadoOperacao $resultado)
    {
        ...
        $nomeArquivo = 'Resultado_Operacao_' . str_replace(' ', '_', $resultado->operacao->nome_operacao) . '_' . now()->format('Y-m-d') . '.pdf';
    ...
    public function dashboard(Request $request)
    {
        ...
        $totalPrisoes = ResultadoOperacao::porUnidade($unidade)->sum('mandados_prisao_cumpridos')
            + ResultadoOperacao::porUnidade($unidade)->sum('prisoes_flagrante');
        
        $totalArmasApreendidas = ResultadoOperacao::porUnidade($unidade)
            ->sum('quantidade_armas_apreendidas');
        
        $totalValoresApreendidos = ResultadoOperacao::porUnidade($unidade)
            ->sum('valores_dinheiro');
